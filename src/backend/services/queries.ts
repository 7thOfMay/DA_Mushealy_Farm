/**
 * SQL queries for fetching data from PostgreSQL smart_farm database.
...
export async function fetchUsers() {
  const rows = await query<UserRow>(`
    SELECT u.user_id, u.username, u.email, u.password_hash, u.full_name, u.phone,
           ...
           STRING_AGG(DISTINCT CAST(uza.zone_id AS TEXT), ',') AS assigned_garden_ids,
           STRING_AGG(DISTINCT CAST(fua.farm_id AS TEXT), ',') AS assigned_farm_ids
    FROM users u
...
export async function insertFarm(
  ...
  const result = await queryOne<{ farm_id: number }>(`
    INSERT INTO farms (farm_code, farm_name, location_desc, status)
    VALUES ($1, $2, $3, 'active')
    RETURNING farm_id
  `, [farmCode, farmName, locationDesc]);
  const insertId = result!.farm_id;

  if (ownerUserId) {
    await query(`
      INSERT INTO farm_user_access (farm_id, user_id) VALUES ($1, $2)
...
export async function fetchGardens() {
  const rows = await query<GardenRow>(`
    SELECT fz.zone_id, fz.farm_id, fz.zone_name, fz.plant_type_id,
           ...
           (SELECT COUNT(*) FROM alerts a WHERE a.zone_id = fz.zone_id AND a.status IN ('detected','processing'))::int AS active_alert_count
    FROM farm_zones fz
  ...
  const rows = await query<DeviceRow>(`
    SELECT d.device_id, d.device_code, d.device_name, d.device_type_id,
           ...
           (SELECT sd.value FROM sensor_data sd
            ...
            ORDER BY sd.recorded_at DESC LIMIT 1) AS latest_value
    FROM devices d
...
export async function updateAlertStatus(
  ...
  if (status === "processing") {
    await query(`
      UPDATE alerts SET status = 'processing', acknowledged_by = $1, acknowledged_at = NOW()
      WHERE alert_id = $2
    `, [userId, alertId]);
  } else {
    await query(`
...
export async function fetchSchedules() {
  const rows = await query<ScheduleRow>(`
    SELECT s.schedule_id, s.zone_id, fz.zone_name, s.device_id, d.device_name,
           s.execution_mode, s.schedule_type, s.start_time::text, s.end_time::text,
...
export async function fetchZoneThresholds() {
  const rows = await query<{ zone_id: number; metric_type: string; min_value: number; max_value: number }>(`
    SELECT zone_id, metric_type, min_value, max_value
    FROM zone_thresholds
    ORDER BY zone_id
  `);

  const zoneMap = new Map<number, Array<{ zone_id: number; metric_type: string; min_value: number; max_value: number }>>();
  for (const r of rows) {
    const list = zoneMap.get(r.zone_id) ?? [];
    list.push(r);
    zoneMap.set(r.zone_id, list);
...
export async function updateFarm(
  ...
  let idx = 1;
  if (patch.farm_name) { sets.push(`farm_name = $${idx++}`); vals.push(patch.farm_name); }
  if (patch.location_desc !== undefined) { sets.push(`location_desc = $${idx++}`); vals.push(patch.location_desc); }
  if (patch.status) { sets.push(`status = $${idx++}`); vals.push(patch.status); }
  if (sets.length === 0) return;
  vals.push(farmId);
  await query(`UPDATE farms SET ${sets.join(", ")} WHERE farm_id = $${idx}`, vals);
}

export async function deleteFarm(farmId: number) {
  await query(`DELETE FROM farm_user_access WHERE farm_id = $1`, [farmId]);
  await query(`DELETE FROM farms WHERE farm_id = $1`, [farmId]);
}
...
export async function insertGarden(
  ...
  const result = await queryOne<{ zone_id: number }>(`
...
export async function updateGarden(
  ...
  let idx = 1;
  if (patch.zone_name) { sets.push(`zone_name = $${idx++}`); vals.push(patch.zone_name); }
  if (patch.plant_type_id !== undefined) { sets.push(`plant_type_id = $${idx++}`); vals.push(patch.plant_type_id); }
  if (patch.area_m2 !== undefined) { sets.push(`area_m2 = $${idx++}`); vals.push(patch.area_m2); }
  if (patch.location_desc !== undefined) { sets.push(`location_desc = $${idx++}`); vals.push(patch.location_desc); }
  if (patch.status) { sets.push(`status = $${idx++}`); vals.push(patch.status); }
  if (sets.length === 0) return;
  vals.push(zoneId);
  await query(`UPDATE farm_zones SET ${sets.join(", ")} WHERE zone_id = $${idx}`, vals);
}

export async function deleteGarden(zoneId: number) {
  await query(`DELETE FROM farm_zones WHERE zone_id = $1`, [zoneId]);
}
...
export async function insertDevice(
  ...
  const result = await queryOne<{ device_id: number }>(`
    ...
    RETURNING device_id
  `, [deviceCode, deviceName, deviceTypeId, zoneId, installLocation, isControllable]);
...
export async function updateDevice(
  ...
  const vals: (string | number)[] = [];
  let idx = 1;
  if (patch.device_name) { sets.push(`device_name = $${idx++}`); vals.push(patch.device_name); }
  if (patch.status) { sets.push(`status = $${idx++}`); vals.push(patch.status); }
  if (patch.install_location !== undefined) { sets.push(`install_location = $${idx++}`); vals.push(patch.install_location); }
  if (sets.length === 0) return;
  sets.push("last_updated = NOW()");
  vals.push(deviceId);
  await query(`UPDATE devices SET ${sets.join(", ")} WHERE device_id = $${idx}`, vals);
}
...
export async function insertSchedule(
  ...
  const result = await queryOne<{ schedule_id: number }>(`
    ...
    VALUES ($1, $2, 'automatic', $3, $4, $5, $6, $7, TRUE, $8)
...
export async function updateSchedule(
  ...
  const sets: string[] = [];
  const vals: (string | number | null | boolean)[] = [];
  let idx = 1;
  if (patch.schedule_type) { sets.push(`schedule_type = $${idx++}`); vals.push(patch.schedule_type); }
  if (patch.start_time) { sets.push(`start_time = $${idx++}`); vals.push(patch.start_time); }
  if (patch.end_time !== undefined) { sets.push(`end_time = $${idx++}`); vals.push(patch.end_time); }
  if (patch.day_of_week !== undefined) { sets.push(`day_of_week = $${idx++}`); vals.push(patch.day_of_week); }
  if (patch.duration_seconds !== undefined) { sets.push(`duration_seconds = $${idx++}`); vals.push(patch.duration_seconds); }
  if (patch.is_active !== undefined) { sets.push(`is_active = $${idx++}`); vals.push(patch.is_active); }
  if (sets.length === 0) return;
  vals.push(scheduleId);
  await query(`UPDATE schedules SET ${sets.join(", ")} WHERE schedule_id = $${idx}`, vals);
}

export async function deleteSchedule(scheduleId: number) {
  await query(`DELETE FROM schedules WHERE schedule_id = $1`, [scheduleId]);
}
...
export async function insertAlertRule(
  ...
  const result = await queryOne<{ alert_rule_id: number }>(`
...
export async function updateAlertRule(
  ...
  const vals: (string | number | null | boolean)[] = [];
  let idx = 1;
  if (patch.rule_name) { sets.push(`rule_name = $${idx++}`); vals.push(patch.rule_name); }
  if (patch.severity) { sets.push(`severity = $${idx++}`); vals.push(patch.severity); }
  if (patch.message_template) { sets.push(`message_template = $${idx++}`); vals.push(patch.message_template); }
  if (patch.is_active !== undefined) { sets.push(`is_active = $${idx++}`); vals.push(patch.is_active); }
  if (sets.length === 0) return;
  vals.push(ruleId);
  await query(`UPDATE alert_rules SET ${sets.join(", ")} WHERE alert_rule_id = $${idx}`, vals);
}

export async function deleteAlertRule(ruleId: number) {
  await query(`DELETE FROM alert_rule_conditions WHERE alert_rule_id = $1`, [ruleId]);
  await query(`DELETE FROM alert_actions WHERE alert_rule_id = $1`, [ruleId]);
  await query(`DELETE FROM alert_rules WHERE alert_rule_id = $1`, [ruleId]);
}
...
export async function updateUser(
  ...
  const vals: (string | number | null | boolean)[] = [];
  let idx = 1;
  if (patch.full_name) { sets.push(`full_name = $${idx++}`); vals.push(patch.full_name); }
  if (patch.phone !== undefined) { sets.push(`phone = $${idx++}`); vals.push(patch.phone); }
  if (patch.role_id) { sets.push(`role_id = $${idx++}`); vals.push(patch.role_id); }
  if (patch.is_active !== undefined) { sets.push(`is_active = $${idx++}`); vals.push(patch.is_active); }
  if (sets.length === 0) return;
  vals.push(userId);
  await query(`UPDATE users SET ${sets.join(", ")} WHERE user_id = $${idx}`, vals);
}

export async function deleteUser(userId: number) {
  await query(`DELETE FROM farm_user_access WHERE user_id = $1`, [userId]);
  await query(`DELETE FROM user_zone_access WHERE user_id = $1`, [userId]);
...
export async function insertBackup(
  ...
  const result = await queryOne<{ backup_id: number }>(`
...
export async function updateBackup(
  ...
  let idx = 1;
  if (patch.status) { sets.push(`status = $${idx++}`); vals.push(patch.status); }
  if (patch.file_size_bytes) { sets.push(`file_size_bytes = $${idx++}`); vals.push(patch.file_size_bytes); }
  if (patch.completed_at) { sets.push("completed_at = NOW()"); }
  if (sets.length === 0) return;
  vals.push(backupId);
  await query(`UPDATE data_backups SET ${sets.join(", ")} WHERE backup_id = $${idx}`, vals);
}
